            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg printable-content">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex items-center space-x-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                        <img class="h-20 w-20 rounded-full object-cover" src="{{ optional($tagihan->santri)->foto ? asset('storage/fotos/' . optional($tagihan->santri)->foto) : '/images/default-avatar.png' }}" alt="Foto Santri">
                        <div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {{-- KODE TANGGAL DIPERBAIKI DI SINI --}}
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Tanggal Tagihan</p>
                                <p class="font-semibold">{{ \Carbon\Carbon::parse($tagihan->tanggal_tagihan)->isoFormat('D MMMM Y') }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Tanggal Jatuh Tempo</p>
                                <p class="font-semibold">{{ \Carbon\Carbon::parse($tagihan->tanggal_jatuh_tempo)->isoFormat('D MMMM Y') }}</p>
                            </div>
                    ...
                    @if($tagihan->status == 'Lunas')
                        <div class="mt-6"><div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Tanggal Pelunasan</p>
                                {{-- KODE TANGGAL DIPERBAIKI DI SINI --}}
                                <p class="font-semibold">{{ $tagihan->tanggal_pelunasan ? \Carbon\Carbon::parse($tagihan->tanggal_pelunasan)->isoFormat('dddd, D MMMM Y') : '-' }}</p>
                            </div>
                        </div></div>
                    @endif
