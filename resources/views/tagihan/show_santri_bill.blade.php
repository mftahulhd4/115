                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-lg font-semibold text-green-600 dark:text-green-400">Rp {{ number_format($tagihan->jenisTagihan->jumlah, 0, ',', '.') }}</dd>
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-end gap-3">
                        <a href="{{ route('tagihan.printReceipt', $tagihan) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-gray-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-600">Cetak Kuitansi</a>
                        <a href="{{ route('tagihan.pdfReceipt', $tagihan) }}" class="inline-flex items-center px-4 py-2 bg-red-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-900">PDF</a>
                        <a href="{{ route('tagihan.editSantriBill', $tagihan) }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600">Edit</a>
                        <form action="{{ route('tagihan.destroySantriBill', $tagihan) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus tagihan ini dari santri tersebut? Aksi ini tidak bisa dibatalkan.');">
                            @csrf
                            @method('DELETE')
